...
- 播放地址：优先使用抓取时提取的 **CDN 直链**（`douyincdn.com` 的 m3u8，
  ...
  - 本轮抓到的在播房间写 CDN 直链（带签名有效期，每 4 小时自动刷新）
...
- 抓取策略（每来源三级自动降级）：
  1. **HTTP 接口**（默认主路径，全程无需浏览器）：分类接口 URL 带 `a_bogus` 参数即可放行
     ...
     直播间用 `room/web/enter`。全量 12 来源实测约 4~5 分钟
...
- m3u 为**增量累积**：本轮房间排最前、重复自动删除、未抓到的历史保留；本轮在播房间写 CDN 直链（有签名时效，4 小时刷新一次），历史房间用 pages.dev 动态地址兜底
